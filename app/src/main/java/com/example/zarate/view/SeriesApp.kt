package com.example.zarate.view
...
import com.example.zarate.data.SerieApiService
import retrofit2.Retrofit
import retrofit2.converter.gson.GsonConverterFactory

@Composable
fun SeriesApp() {
    val urlBase = "http://192.168.0.6:8000/" // Cambia a tu IP o 10.0.2.2 si usas un emulador
    ...
    Box(
        ...
        NavHost(
            ...
            composable("serieVer/{id}", arguments = listOf(
                ...
                ContenidoSerieEditar(navController, servicio, id)
            }
            composable("serieDel/{id}", arguments = listOf(
                navArgument("id") { type = NavType.IntType }
            )) {
                val id = it.arguments?.getInt("id") ?: 0
                ContenidoSerieEliminar(navController, servicio, id)
            }
        }
    }
}
